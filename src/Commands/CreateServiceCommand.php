...
{
    protected $signature = 'layerize:service {serviceName} {--all|all : Create all service file include QueryService, DatatableService, CommandService}';
    ...
    public function handle()
    {
        ...
        if (!$this->option('all')) {
            ...
            if (File::exists($servicePath)) {
                $this->line('');
                $this->line(sprintf('<bg=red;fg=black>%s</>', ' ERROR ') . ' ' . "File Service '{$serviceName}.php' already exists");
                $this->line('');
                return;
            }
